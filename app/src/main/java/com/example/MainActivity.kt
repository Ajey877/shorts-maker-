        ...
        Scaffold(
          ...
          Box(Modifier.fillMaxSize().padding(innerPadding)) {
            when (uiState.activeTab) {
              0 -> HomeScreen(
                urlInput = uiState.urlInput, isAnalyzing = uiState.isAnalyzing, analysisStatusText = uiState.analysisStatusText,
                currentVideo = uiState.currentVideo, clips = uiState.clips, selectedClip = uiState.selectedClip,
                retentionPoints = uiState.retentionPoints, isPlaying = uiState.isPlaying, playbackPositionSec = uiState.playbackPositionSec,
                captionStyle = uiState.captionStyle, framingMode = uiState.framingMode, customHookHeadline = uiState.customHookHeadline,
                savedClipsCount = savedClips.size, onUrlInputChanged = viewModel::onUrlInputChanged, onAnalyzeClicked = { viewModel.analyzeVideo() },
                onLoadPreset = viewModel::loadPreset, onSelectClip = viewModel::selectClip, onTogglePlayPause = viewModel::togglePlayPause,
                onSeek = viewModel::seekTo, onCaptionStyleChanged = viewModel::setCaptionStyle, onFramingModeChanged = viewModel::setFramingMode,
                onHookHeadlineChanged = viewModel::onCustomHookChanged, onTrimUpdated = viewModel::updateClipTrim, onSaveClip = viewModel::saveCurrentClip,
                ...
                onShareClip = { clip -> viewModel.shareShortsClip(context, clip) }, onOpenLibrary = { viewModel.setActiveTab(1) }
              )
              1 -> LibraryScreen(
                ...
                onTogglePostedStatus = viewModel::togglePostedStatus, onDeleteClip = viewModel::deleteSavedClip, onGoToStudio = { viewModel.setActiveTab(0) }
              )
            }
            ...
            AnimatedVisibility(visible = uiState.bannerNotification != null, enter = slideInVertically { -it } + fadeIn(), exit = slideOutVertically { -it } + fadeOut(), modifier = Modifier.align(Alignment.TopCenter).padding(top = 16.dp, start = 20.dp, end = 20.dp)) {
              uiState.bannerNotification?.let { msg ->
                Surface(shape = RoundedCornerShape(12.dp), color = BrightCrimson, shadowElevation = 8.dp) {
                  Row(Modifier.padding(horizontal = 14.dp, vertical = 10.dp), verticalAlignment = Alignment.CenterVertically) {
                    Icon(Icons.Default.Info, null, tint = Color.White, modifier = Modifier.size(18.dp)); Spacer(Modifier.width(8.dp)); Text(msg, color = Color.White, fontSize = 13.sp, fontWeight = FontWeight.Bold)
                  }
                }
              }
            }
            ...
            if (uiState.showUploadDialog && uiState.selectedClip != null) {
              ExportUploadDialog(
                ...
                isLocalMedia = uiState.currentVideo?.isLocalMedia == true,
                isExporting = uiState.isExporting,
                hasExportedClip = uiState.exportedClipUri != null,
                onDismiss = { viewModel.setUploadDialogVisible(false) },
                onExport = { viewModel.exportCurrentClip() },
                onShareExported = { viewModel.shareExportedClip(context) },
